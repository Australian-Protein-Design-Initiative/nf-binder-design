...
# TODO: Consider using this fork: https://github.com/ullahsamee/IPSAE
#       and integrating this Boltz-2 support PR: https://github.com/DunbrackLab/IPSAE/pull/21
#       Not yet released - ipSAE_min code from Overath et al, 2025 https://www.biorxiv.org/content/10.1101/2025.08.14.670059v1.full
#       (could implement from description in paper)
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate ipSAE score for scoring pairwise protein-protein interactions in AlphaFold2, AlphaFold3, and Boltz1 models.",
        epilog=(
            "Examples:\n"
            "  AF2:   python ipsae.py <path_to_pae_json_file> <path_to_pdb_file> <pae_cutoff> <dist_cutoff>\n"
            ...
            "  AF3:   python ipsae.py <path_to_pae_json_file> <path_to_mmcif_file> <pae_cutoff> <dist_cutoff>\n"
            "         python ipsae.py fold_aurka_tpx2_full_data_0.json fold_aurka_tpx2_model_0.cif 10 10\n"
            "  Boltz1/2: python ipsae.py <path_to_pae_npz_file> <path_to_structure_file> <pae_cutoff> <dist_cutoff>\n"
            "         python ipsae.py pae_AURKA_TPX2_model_0.npz AURKA_TPX2_model_0.cif 10 10\n"
            "         python ipsae.py pae_AURKA_TPX2_model_0.npz AURKA_TPX2_model_0.pdb 10 10"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "pae_file",
        help="Path to PAE file (JSON for AF2/AF3, NPZ for Boltz1/2)",
    )
    parser.add_argument(
        "structure_file",
        help="Path to structure file (PDB for AF2/Boltz1, CIF for AF3/Boltz1/2)",
    )
    parser.add_argument(
        "pae_cutoff",
        type=float,
        nargs="?",
        default=10.0,
        help="PAE cutoff value (default: 10.0)",
    )
    parser.add_argument(
        "dist_cutoff",
        type=float,
    ...
    for chain1 in unique_chains:
        for chain2 in unique_chains:
            if chain1 == chain2:
                continue
            if (
                chain_dict[chain1] == "nucleic_acid"
                or chain_dict[chain2] == "nucleic_acid"
            ):
    ...
    for chain1 in unique_chains:
        for chain2 in unique_chains:
            ...
            d0res_byres_all = calc_d0_array(
                n0res_byres_all, chain_pair_type[chain1][chain2]
            )
            ...
            for i in range(numres):
                ...
                out_fields = [
                    ...
                    str(chain1),
                    str(chain2),
                    str(residues[i]["resnum"]),
                    str(residues[i]["res"]),
                    f"{plddt[i]:.2f}",
                    str(int(n0chn[chain1][chain2])),
                    str(int(n0dom[chain1][chain2])),
                    str(int(n0res_byres[chain1][chain2][i])),
    ...
    for chain1 in unique_chains:
        for chain2 in unique_chains:
            ...
            iptm_d0chn_asymres[chain1][chain2] = (
                residues[max_index]["residue"] if max_index is not None else "None"
            )
            ...
            ipsae_d0dom_asymres[chain1][chain2] = (
                residues[max_index]["residue"] if max_index is not None else "None"
            )
            ...
            ipsae_d0res_asymres[chain1][chain2] = (
                residues[max_index]["residue"] if max_index is not None else "None"
            )
            ...
            if chain1 > chain2:
                maxvalue = max(
                    iptm_d0chn_asym[chain1][chain2], iptm_d0chn_asym[chain2][chain1]
                )
                ...
                if minvalue == iptm_d0chn_asym[chain1][chain2]:
                    minres = iptm_d0chn_asymres[chain1][chain2]
                else:
                    minres = iptm_d0chn_asymres[chain2][chain1]
                ...
                maxvalue = max(
                    ipsae_d0chn_asym[chain1][chain2], ipsae_d0chn_asym[chain2][chain1]
                )
                ...
                if minvalue == ipsae_d0chn_asym[chain1][chain2]:
                    minres = ipsae_d0chn_asymres[chain1][chain2]
                else:
                    minres = ipsae_d0chn_asymres[chain2][chain1]
                ...
                maxvalue = max(
                    ipsae_d0dom_asym[chain1][chain2], ipsae_d0dom_asym[chain2][chain1]
                )
                ...
                maxvalue = max(
                    ipsae_d0res_asym[chain1][chain2], ipsae_d0res_asym[chain2][chain1]
                )
        ...
        for pair in (pair1, pair2):
            ...
            if chain1 > chain2:
                ...
                out_fields = [
                    ...
                    f"{pDockQ2_value:.4f}",
                    f"{LIS_Score:.4f}",
                    str(int(n0res_max[chain1][chain2])),
                    str(int(n0chn[chain1][chain2])),
                    str(int(n0dom_max[chain1][chain2])),
                ...
                dist_residues_1_min = min(
                    len(dist_unique_residues_chain2[chain1][chain2]),
                    len(dist_unique_residues_chain1[chain2][chain1]),
                )
                dist_residues_2_min = min(
                    len(dist_unique_residues_chain1[chain1][chain2]),
                    len(dist_unique_residues_chain2[chain2][chain1]),
                )
                ...
                out_fields_min = [
                    ...
                    str(dist_string),
                    "min",
                    f"{ipsae_d0res_min[chain1][chain2]:.6f}",
                    f"{ipsae_d0chn_min[chain1][chain2]:.6f}",
                    f"{ipsae_d0dom_min[chain1][chain2]:.6f}",
                    f"{iptm_af_value_min:.3f}",
                    f"{iptm_d0chn_min[chain1][chain2]:.6f}",
                    f"{pDockQ[chain1][chain2]:.4f}",
                    f"{pDockQ2_value_min:.4f}",
                    f"{LIS_Score_min:.4f}",
                    str(int(n0res_min[chain1][chain2])),
                    str(int(n0chn[chain1][chain2])),
                    str(int(n0dom_min[chain1][chain2])),
                    f"{d0res_min[chain1][chain2]:.2f}",
                    f"{d0chn[chain1][chain2]:.2f}",
                    f"{d0dom_min[chain1][chain2]:.2f}",
                    str(residues_1_min),
                    str(residues_2_min),
                    str(dist_residues_1_min),
                    str(dist_residues_2_min),
                    str(pdb_stem),
                ]
                outstring_min = "\t".join(out_fields_min) + "\n"
                OUT.write(outstring_min)
                PML.write("# " + outstring_min)
