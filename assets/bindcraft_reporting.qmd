...
#| fig-height: 8
        ...
        if trajectory_df is not None and len(trajectory_df) > 0:
            ...
            if available_traj_metrics:
                ...
                if plot_data:
                    # Create box plot
                    bp = ax.boxplot(plot_data, patch_artist=True, tick_labels=labels)
            ...
            if 'Target_RMSD' in trajectory_df.columns:
                ...
                if len(rmsd_data) > 0:
                    ...
                    bp = ax.boxplot(rmsd_data.values, patch_artist=True, tick_labels=['Target_RMSD'])
            ...
            if 'dG' in trajectory_df.columns:
                ...
                if len(dg_data) > 0:
                    ...
                    bp = ax.boxplot(dg_data.values, patch_artist=True, tick_labels=['dG'])
        ...
        if mpnn_design_df is not None and len(mpnn_design_df) > 0:
            ...
            if 'Average_dG' in mpnn_design_df.columns:
                ...
                if len(dg_data) > 0:
                    ...
                    # Create box plot for dG
                    bp = ax.boxplot(dg_data.values, patch_artist=True, tick_labels=['Average_dG'])
...
if final_design_df is not None and len(final_design_df) > 0:
    ...
    if tiled_metrics:
        ...
        for i, metric in enumerate(tiled_metrics):
            ...
            if len(data) > 0:
                # Create box plot
                bp = ax.boxplot(data.values, patch_artist=True, tick_labels=[metric])
    ...
    if standalone_plots:
        ...
        for i, (plot_type, title) in enumerate(standalone_plots):
            ...
            if plot_type == "clashes_group":
                ...
                if len(data) > 0:
                    bp = ax.boxplot(data.values, patch_artist=True, tick_labels=[plot_type])
                    bp["boxes"][0].set_facecolor("lightcoral")
                    bp["medians"][0].set_color("red")
