...
/*
BindCraft binder design workflow

Usage via main.nf:
  nextflow run main.nf --method bindcraft --input_pdb target.pdb --target_chains A
...
workflow BINDCRAFT {
    ...
    ch_final_stats_merged = BINDCRAFT_PROCESS.out.final_stats_csv
        .collectFile(name: 'final_design_stats.csv',
                     storeDir: "${params.outdir}/bindcraft",
                     keepHeader: true,
                     skip: 1)    

    ch_trajectory_stats_merged = BINDCRAFT_PROCESS.out.trajectory_stats_csv
        .collectFile(name: 'trajectory_stats.csv',
                     storeDir: "${params.outdir}/bindcraft",
                     keepHeader: true,
                     skip: 1)

    ch_mpnn_design_stats_merged = BINDCRAFT_PROCESS.out.mpnn_design_stats_csv
    ...
    ch_failure_csv_merged = BINDCRAFT_PROCESS.out.failure_csv
        .collect()
        .map { files ->
            def header = ''
            def sums = [:]
    ...
    ch_batch_dirs_list = BINDCRAFT_PROCESS.out.batch_dir.collect()
