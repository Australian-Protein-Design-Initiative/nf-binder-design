...
- **`--rf3_target_fasta`**  
  Optional path to a target FASTA file used when building the RF3 target MSA (only when `--rf3_create_target_msa` is set and `--rf3_alignment` is not). If unset, the query sequence is taken from **`--input_pdb`** / config `input` (chain **A**), or from **`--rf3_target_template`** (target chain ID) when that option is set. **Mutually exclusive with `--rf3_target_template`** (you cannot pass both flags).

- **`--rf3_target_template`**  
  Path to an alternative PDB or CIF to use as the RosettaFold3 template instead of the default structure from **`--input_pdb`** / the `--rfd3_config` `input` path. This file is **not** trimmed to contigs. **Single-chain file:** the chain is renamed to the RFD3 **target chain** if needed. **Multi-chain file:** only the chain whose ID already matches the RFD3 target chain is kept (other chains are dropped). The target sequence RF3 uses is taken from this template (RF3 does not allow a separate target `seq` component alongside the same chain in the template). **Mutually exclusive with `--rf3_target_fasta`.** When **`--rf3_create_target_msa`** is set (and **`--rf3_alignment`** is not), the workflow builds the target MSA from this template file using the **target chain** ID inferred from the contig / config, so the MSA width matches the template sequence.

- **`--rf3_template_selection`**  
  Comma-separated RF3 `template_selection` AtomSelection tokens (see [RF3 templating](https://rosettacommons.github.io/foundry/models/rf3/)), controlling which regions are templated vs predicted. Examples: `A` for the whole target chain, or `A/*/1-50,A/*/80-120` for partial templating. **Default:** the target chain letter only (whole target chain).
