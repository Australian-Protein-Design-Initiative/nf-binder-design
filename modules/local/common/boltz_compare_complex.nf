process BOLTZ_COMPARE_COMPLEX {
    ...
    input:
    // Staged as design_inputs/input_00001, input_00002, ... rather than under their own
    // names: a Nextflow collection will not stage two files with the same basename, and
    // RFD3 hands every design's RF3 output over as `model.cif`. Each `?` is a zero-padded
    // index (five digits, so a listing stays in batch order up to 99999). A `*` here would
    // be a second, unpadded counter. The real extensions come alongside in pdb_exts, since
    // the wildcard discards them. Names that leave the task are rebuilt from the design id.
    tuple val(metas), path(pdbs, stageAs: 'design_inputs/input_?????'), val(pdb_exts)
    val binder_chain
    val target_chain
