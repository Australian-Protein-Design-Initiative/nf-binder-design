process BOLTZ_COMPARE_BINDER_MONOMER {
    ...
    // Boltz complex outputs are always .pdb. Names that leave the task are rebuilt from
    // the design id.
    tuple val(metas), path(af2ig_pdbs, stageAs: 'design_inputs/input_?????'), path(boltz_complex_pdbs, stageAs: 'complex_inputs/complex_?????.pdb'), val(design_exts)
    val binder_chain
...
NFBD_MANIFEST_EOF
    ...
    while IFS=\$'\\t' read -r design_id staged_design complex_pdb design_ext; do
        ...
        # Staged basename is complex_00001.pdb; structure1 has to be the design id.
        ln -s "\$(readlink -f "\$complex_pdb")" "fixed/\${design_id}_complex.pdb"
        ln -s "\$(readlink -f "\$MONOMER_PDB")" "mobile/\$(basename "\$MONOMER_PDB")"
